...
export class SeismicSentinel {
  ...
  private async poll(): Promise<void> {
    ...
    const cachedJmaFp = await readTextCheckpoint(bucket, 'feed/_fp_jma.txt');
    ...
    let jmaPersisted = false;
    try {
      const { persisted } = await pollJma(this.env, db, jmaEvents);
      jmaPersisted = persisted;
    } catch (err) {
    ...
    if (!jmaPersisted) {
      console.error('[sentinel] checkpoint not advanced because ingest did not fully persist');
      return;
    }
    ...
    // Only publish events + governor (not maritime/rail — those run on cron heartbeat)
    try {
      const checkpointResult = await advanceCheckpointAfterSuccess({
        currentCheckpoint: cachedJmaFp,
        nextCheckpoint: jmaFp,
        performWork: async () => {
          if (!jmaPersisted) {
            return false;
          }
          await publishCoreFeeds(this.env, db, governorEnvelope);
          return true;
        },
      ...
      if (checkpointResult === 'held') {
        console.error('[sentinel] checkpoint held for retry');
        return;
      }
    } catch (err) {
      console.error('[sentinel] R2 publish/checkpoint failed:', err);
      return;
    }
