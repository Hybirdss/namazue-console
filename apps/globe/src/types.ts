...
export type MetricStatus = 'ok' | 'hidden';
export type MetricClaim = 'official' | 'catalog' | 'modeled';
export type HazardFieldSource =
  | 'shakemap'
  | 'gmpe_grid'
  | 'scenario_official'
  | 'scenario_modeled';
export type ScenarioMetricUnit =
  | 'text'
  | 'percent'
  | 'years'
  | 'jma'
...
export type EarthquakeSource = 'server' | 'usgs' | 'jma' | 'historical' | 'scenario';
...
export interface EarthquakeEvent {
  ...
  /** JMA observed max intensity (shindo), e.g. "5+", "4", "3". Null for non-JMA events. */
  observedIntensity?: string | null;
  /** Moment tensor nodal-plane-1 strike (degrees), from USGS focal mechanism. */
  mtStrike?: number | null;
  scenarioId?: string | null;
  scenarioFaultId?: string | null;
...
export interface GmpeInput {
  Mw: number;          // Moment magnitude (engine applies its configured cap internally)
...
export interface GmpeResult {
  ...
  pgv_surface: number;  // PGV at the engine's default site condition after amplification
...
export interface TsunamiArrivalEstimate {
  portName: string;
  portNameJa: string;
  distanceKm: number;
...
export interface TsunamiSummary {
  risk: TsunamiAssessment['risk'];
  status: 'none' | 'modeled' | 'official';
  source: HazardFieldSource | 'catalog';
  arrivalEstimatesMin: TsunamiArrivalEstimate[];
  heightMeters: number | null;
  confidence: TsunamiAssessment['confidence'];
  factors: string[];
}

export interface HazardField {
  source: HazardFieldSource;
