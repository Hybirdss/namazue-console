 ...
 * Per-locale naming for sensitive regions.
...
const GEO_TABLE: GeoEntry[] = [
  ...
  // ── Sea regions with per-locale naming ──
  // Each locale uses its own internationally recognized standard name.
  { lat: 38.5, lng: 134.0, ja: '日本海中部', en: 'Off western Honshu', ko: '동해 중부', offshore: true },
];
 ...
 */
/**
 * Get a localized place name for display, given an event's coordinates
 * and the raw USGS place text as fallback.
 *
 * Uses the current i18n locale to select ja/en/ko.
 * Falls back to USGS text if coordinates are outside Japan coverage.
 */
export function getLocalizedPlace(lat: number, lng: number, usgsText: string): string {
  const locale = getLocale();
  const names = getJapanPlaceName(lat, lng);
  if (!names) return usgsText;
  return names[locale] || names.en || usgsText;
}
