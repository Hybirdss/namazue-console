...
// Status colors — used in ring and label layers
...
export function createHospitalLayers(
  ...
  // Status rings — visual severity indicator behind colored icons
  const statusData = data.filter(d =>
    d.posture !== 'operational' || d.dmatDeployable || d.inZone,
  );
  if (statusData.length > 0) {
    layers.push(new ScatterplotLayer<HospitalDatum>({
      id: 'hospital-status-rings',
      data: statusData,
      pickable: false,
      stroked: true,
      filled: true,
      ...
      getRadius: 16,
      getFillColor: (d): RGBA => {
        if (d.posture === 'compromised') return [239, 68, 68, 40];
        if (d.posture === 'assessment-needed') return [251, 146, 60, 30];
        if (d.posture === 'disrupted') return [251, 191, 36, 25];
        if (d.dmatDeployable) return [34, 211, 238, 30];
        if (d.inZone) return [239, 68, 68, 30];
        return [0, 0, 0, 0];
      },
      getLineColor: (d): RGBA => {
        if (d.posture === 'compromised') return [239, 68, 68, 180];
        if (d.posture === 'assessment-needed') return [251, 146, 60, 160];
        if (d.posture === 'disrupted') return [251, 191, 36, 140];
        if (d.dmatDeployable) return [34, 211, 238, 160];
  ...
  if (zoom >= 8) {
    const labelData = pickVisibleLabels(data, {
      ...
      getPriority: (datum) => {
        if (datum.posture === 'compromised') return 200;
        if (datum.dmatDeployable) return 160;
        if (datum.posture === 'assessment-needed') return 140;
        if (datum.inZone) return 120;
        if (datum.dmat) return 90;
        return 20;
      },
    });
