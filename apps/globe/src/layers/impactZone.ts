...
const impactRadiusCache = new Map<string, number>();

function buildImpactRadiusCacheKey(
  magnitude: number,
  depth_km: number,
  faultType: FaultType,
): string {
  return `${magnitude.toFixed(2)}|${depth_km.toFixed(1)}|${faultType}`;
}
...
export function impactRadiusKm(
  ...
  const cacheKey = buildImpactRadiusCacheKey(magnitude, depth_km, faultType);
  const cached = impactRadiusCache.get(cacheKey);
  if (cached !== undefined) {
    return cached;
  }
  ...
  impactRadiusCache.set(cacheKey, radius);
  return radius;
}
