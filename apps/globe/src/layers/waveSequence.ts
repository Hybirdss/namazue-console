...
// Uses actual depth and fault type so deep/intraslab events don't overestimate reach.
function maxRadiusForEvent(magnitude: number, depth_km: number, faultType: FaultType): number {
  return jmaThresholdDistanceKm(magnitude, depth_km, faultType);
}
...
export function createInactiveSequence(): WaveSequenceState {
  return { active: false, startTime: 0, epicenter: { lat: 0, lng: 0 }, magnitude: 0, depth_km: 0, faultType: 'crustal' };
}
...
export function startSequence(
  ...
  return { active: true, startTime: Date.now(), epicenter, magnitude, depth_km, faultType };
}
...
export function getSWaveRadiusKm(state: WaveSequenceState, now: number): number {
  ...
  return Math.min(VS_VISUAL_KMS * sElapsedSec, maxRadiusForEvent(state.magnitude, state.depth_km, state.faultType));
}
...
export function createSequenceLayers(state: WaveSequenceState, now: number): Layer[] {
  ...
  const maxRKm = maxRadiusForEvent(state.magnitude, state.depth_km, state.faultType);
