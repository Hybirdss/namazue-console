...
// Blob URL avoids CSP connect-src blocking of data: URIs fetched via fetch()
const ICON_BLOB = new Blob([SHIP_SVG], { type: 'image/svg+xml' });
...
export function computeMaritimeExposure(
  ...
  const parts: string[] = [tf('ais.inZoneSummary', { n: total })];
  if (passenger > 0) parts.push(tf('ais.passengerCount', { n: passenger }));
  if (tanker > 0) parts.push(tf('ais.tankerCount', { n: tanker }));
...
export function formatVesselTooltip(v: Vessel, selectedEvent: EarthquakeEvent | null): string {
  const typeLabel = t(`exposure.shipType.${v.type}`).toUpperCase();
  const speed = v.sog > 0.5 ? `${v.sog.toFixed(1)} kn` : t('ais.anchored');
  const heading = v.sog > 0.5 ? `${t('ais.heading')} ${Math.round(v.cog)}°` : '';
  const priority = isHighPriorityVessel(v.type) ? ` — ${t('ais.highPriorityLabel')}` : '';
  ...
  if (selectedEvent) {
    ...
    const dist = haversineKm(v.lat, v.lng, selectedEvent.lat, selectedEvent.lng);
    if (dist <= radius) {
      zoneInfo = `<div style="color:#ef4444;font-weight:600;margin-top:4px">${tf('ais.impactZoneWarning', { dist: Math.round(dist) })}</div>`;
    }
  }

  return `
    <div style="font-weight:600;font-size:12px;margin-bottom:3px">${escapeHtml(v.name)}${priority}</div>
