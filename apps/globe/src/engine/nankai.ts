...
import { DEFAULT_SITE_AMP_FACTOR, pgvSurfaceToJmaIntensity } from './gmpe';
...
function bufferToIntensityGrid(
  ...
  for (let i = 0; i < view.length; i++) {
    ...
    const pgvSurface = Math.sqrt(sumPgvSq) * DEFAULT_SITE_AMP_FACTOR;
    const intensity = pgvSurfaceToJmaIntensity(pgvSurface);
...
export async function runNankaiScenario(
  ...
  // SharedArrayBuffer unavailable = sequential fallback (slower but functional)
