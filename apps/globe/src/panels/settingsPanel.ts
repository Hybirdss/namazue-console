...
function getMethodologyReferences(): {
	...
	return {
		academic: [
			...
			{
				label:
					"Midorikawa, Fujimoto & Muramatsu (1999) — PGV to JMA intensity correlation",
			...
			{
				label:
					"Abrahamson, Silva & Kamai (2014) — NGA-West2 ground-motion model",
			...
			{
				label:
					"Campbell & Bozorgnia (2014) — NGA-West2 horizontal PGA/PGV model",
...
function renderGeneralSettingsMarkup(prefs: ConsolePreferences): string {
	const shortcutRows = getShortcutSections()
		.map((section) => {
	...
	const magOptions = MAG_OPTIONS.map(
		(opt) =>
...
export function renderSettingsMarkup(
	...
	const generalActive =
		activeTab === "general" ? " nz-settings__tab-btn--active" : "";
	const methodologyActive =
		activeTab === "methodology" ? " nz-settings__tab-btn--active" : "";
	const bodyMarkup =
...
export function createSettingsPanel(
	...
	function bindInteractions(): void {
		...
		overlay
			.querySelector('[data-setting="notifications-enabled"]')
		...
		overlay
			.querySelector<HTMLSelectElement>('[data-setting="notifications-mag"]')
			?.addEventListener("change", (e) => {
				prefs.notifications.minMagnitude = Number(
		...
		// Sound toggle
		overlay
			.querySelector('[data-setting="notifications-sound"]')
			?.addEventListener("click", () => {
		...
		// Keyboard toggle
		overlay
			.querySelector('[data-setting="keyboard-enabled"]')
			?.addEventListener("click", () => {
		...
		// Display coordinates
		overlay
			.querySelector('[data-setting="display-coordinates"]')
			?.addEventListener("click", () => {
	...
	function open(): void {
		...
		requestAnimationFrame(() =>
			overlay.classList.add("nz-settings-overlay--open"),
		);
	}
