...
describe('impact intelligence markup', () => {
  it('keeps statistical source attribution while hiding calculation methodology from the main panel', () => {
    ...
    expect(markup).toContain('Consequence Matrix');
    expect(markup).toContain('Peak Intensity');
    expect(markup).toContain('Infrastructure');
    expect(markup).toContain('MIC Resident Registry (2025-01-01)');
    expect(markup).toContain('Beta information.');
    expect(markup).toContain('Verify official releases and primary sources');
  ...
  it('does not render modeled area values with K km² shorthand in the operator panel', () => {
    const markup = renderImpactIntelligenceMarkup(createIntelligence(), createEvent());

    expect(markup).not.toContain('K km²');
  });

  it('renders threshold-specific exposed population for municipalities instead of the full municipality population', () => {
    const intel = createIntelligence();
    intel.populationExposure!.topMunicipalitiesByThreshold = {
      '7': [],
      '6+': [
        {
          name: 'Yokohama',
          nameEn: 'Yokohama',
          population: 820_000,
          exposedPopulation: 210_000,
          intensity: 6.1,
          jmaClass: '6+',
        },
      ],
      '6-': intel.populationExposure!.topAffected,
      '5+': intel.populationExposure!.topAffected,
      '5-': intel.populationExposure!.topAffected,
      '4+': intel.populationExposure!.topAffected,
      '3+': intel.populationExposure!.topAffected,
    };

    const markup = renderImpactIntelligenceMarkup(intel, createEvent());

    expect(markup).toContain('210K');
    expect(markup).not.toContain('820K');
  });
});
