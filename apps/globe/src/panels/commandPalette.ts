...
const LOCATIONS: FlyTarget[] = [
  { labelKey: 'location.tokyo', aliases: ['東京', 'tokyo', 'とうきょう', '도쿄'], lat: 35.6812, lng: 139.7671, zoom: 10 },
  { labelKey: 'location.osaka', aliases: ['大阪', 'osaka', 'おおさか', '오사카'], lat: 34.6937, lng: 135.5023, zoom: 10 },
  { labelKey: 'location.nagoya', aliases: ['名古屋', 'nagoya', 'なごや', '나고야'], lat: 35.1815, lng: 136.9066, zoom: 10 },
  { labelKey: 'location.sendai', aliases: ['仙台', 'sendai', 'せんだい', '센다이'], lat: 38.2682, lng: 140.8694, zoom: 10 },
  { labelKey: 'location.sapporo', aliases: ['札幌', 'sapporo', 'さっぽろ', '삿포로'], lat: 43.0621, lng: 141.3544, zoom: 10 },
  { labelKey: 'location.fukuoka', aliases: ['福岡', 'fukuoka', 'ふくおか', '후쿠오카'], lat: 33.5904, lng: 130.4017, zoom: 10 },
  { labelKey: 'location.hiroshima', aliases: ['広島', 'hiroshima', 'ひろしま', '히로시마'], lat: 34.3853, lng: 132.4553, zoom: 10 },
  { labelKey: 'location.kobe', aliases: ['神戸', 'kobe', 'こうべ', '고베'], lat: 34.6901, lng: 135.1956, zoom: 10 },
  { labelKey: 'location.yokohama', aliases: ['横浜', 'yokohama', 'よこはま', '요코하마'], lat: 35.4437, lng: 139.6380, zoom: 10 },
  { labelKey: 'location.kyoto', aliases: ['京都', 'kyoto', 'きょうと', '교토'], lat: 35.0116, lng: 135.7681, zoom: 10 },
  { labelKey: 'location.niigata', aliases: ['新潟', 'niigata', 'にいがた', '니가타'], lat: 37.9026, lng: 139.0236, zoom: 10 },
  { labelKey: 'location.kagoshima', aliases: ['鹿児島', 'kagoshima', 'かごしま', '가고시마'], lat: 31.5966, lng: 130.5571, zoom: 10 },
  { labelKey: 'location.naha', aliases: ['那覇', 'okinawa', '沖縄', 'naha', 'なは', '나하'], lat: 26.3344, lng: 127.7671, zoom: 10 },
  { labelKey: 'location.kumamoto', aliases: ['熊本', 'kumamoto', 'くまもと', '구마모토'], lat: 32.7898, lng: 130.7417, zoom: 10 },
  { labelKey: 'location.hakodate', aliases: ['函館', 'hakodate', 'はこだて', '하코다테'], lat: 41.7687, lng: 140.7290, zoom: 10 },
  { labelKey: 'location.shizuoka', aliases: ['静岡', 'shizuoka', 'しずおか', '시즈오카'], lat: 34.9756, lng: 138.3827, zoom: 10 },
  { labelKey: 'location.kanazawa', aliases: ['金沢', 'kanazawa', 'かなざわ', '가나자와'], lat: 36.5613, lng: 136.6562, zoom: 10 },
  { labelKey: 'location.matsuyama', aliases: ['松山', 'matsuyama', 'まつやま', '마쓰야마'], lat: 33.8395, lng: 132.7657, zoom: 10 },
  { labelKey: 'location.nankaiTrough', aliases: ['南海トラフ', 'nankai', 'なんかい', '난카이'], lat: 33.5, lng: 136.0, zoom: 6 },
  { labelKey: 'location.sagamiTrough', aliases: ['相模トラフ', 'sagami', 'さがみ', '사가미'], lat: 34.5, lng: 139.5, zoom: 7 },
  { labelKey: 'location.japanOverview', aliases: ['日本', 'japan', 'にほん', 'overview', '全国', '일본'], lat: 36.5, lng: 137.5, zoom: 5.5 },
];
...
function buildCommands(flyTo: (lat: number, lng: number, zoom: number) => void): PaletteCommand[] {
  ...
  for (const loc of LOCATIONS) {
    ...
    commands.push({
      id: `goto-${loc.labelKey.replace('location.', '')}`,
      label,
      aliases: [...loc.aliases, label.toLowerCase()],
      category: 'location',
      icon: '◎',
  ...
  for (const layer of getAllLayerDefinitions()) {
    ...
    commands.push({
      ...
      hint: () => {
        ...
        return vis[layer.id] ? t('palette.visible') : t('palette.hidden');
      },
      execute: () => {
  ...
  commands.push({
    id: 'action-scenario',
    label: t('palette.toggleScenario'),
...
function buildEventCommands(
  ...
  return events.slice(0, 20).map((event) => ({
    id: `event-${event.id}`,
    label: escapeHtml(getLocalizedPlace(event.lat, event.lng, event.place.text)),
    aliases: [event.id, event.place.text.toLowerCase(), getLocalizedPlace(event.lat, event.lng, event.place.text).toLowerCase(), `m${event.magnitude.toFixed(1)}`],
    category: 'event',
    icon: '●',
...
function renderPalette(
  ...
  if (results.length === 0 && query.length > 0) {
    return `<div class="nz-palette__empty">${t('palette.noResults')}</div>`;
  }
  if (results.length === 0) {
    return `<div class="nz-palette__hint">${t('palette.hintDefault')}</div>`;
  }
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (cmd.category !== currentCategory) {
      ...
      const categoryLabel = t(`palette.category.${currentCategory}`) || currentCategory;
      items.push(`<div class="nz-palette__category">${categoryLabel}</div>`);
    }
