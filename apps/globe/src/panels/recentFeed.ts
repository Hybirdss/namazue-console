...
import { fetchEventsByRange } from '../data/eventFeed';
...
function formatTimeAgo(timestamp: number): string {
  ...
  if (diff < 60_000) return t('feed.timeNow');
  if (diff < 3600_000) return tf('feed.timeMin', { n: Math.floor(diff / 60_000) });
  if (diff < 86400_000) return tf('feed.timeHr', { n: Math.floor(diff / 3600_000) });
  const days = Math.floor(diff / 86400_000);
  if (days < 30) return tf('feed.timeDay', { n: days });
  if (days < 365) return tf('feed.timeMonth', { n: Math.floor(days / 30) });
  const years = Math.floor(days / 365);
  const months = Math.floor((days % 365) / 30);
  if (months > 0) return tf('feed.timeYearMonth', { y: years, m: months });
  return tf('feed.timeYear', { n: years });
}
...
function depthLabel(km: number): string {
  if (km <= 30) return t('feed.depth.shallow');
  if (km <= 70) return `${Math.round(km)}km`;
  return t('feed.depth.deep');
}
...
function renderPeriodPicker(
  ...
  const picker = pickerOpen ? `
    <div class="nz-feed__range-picker">
      ...
      <div class="nz-feed__range-row">
        <label class="nz-feed__range-label">${t('feed.to')}</label>
      ...
      <div class="nz-feed__range-actions">
        <span class="nz-feed__range-hint">${t('feed.maxYear')}</span>
        <button class="nz-feed__range-apply" data-action="apply-range"${loading ? ' disabled' : ''}>${t('feed.apply')}</button>
        ${customRange ? `<button class="nz-feed__range-clear" data-action="clear-range">${t('feed.clear')}</button>` : ''}
      </div>
    </div>
  ` : '';
...
function renderFeed(
  ...
  const items = visible.map((e) => {
    ...
    const tsunami = e.tsunami ? `<span class="nz-feed__tsunami">${t('feed.tsunami')}</span>` : '';
    return `
      <div class="nz-feed__item${active}" data-event-id="${e.id}">
        ...
        <span class="nz-feed__place">${escapeHtml(getLocalizedPlace(e.lat, e.lng, e.place.text))}${tsunami}</span>
  ...
  const moreLabel = filtered.length > MAX_FEED_ITEMS
    ? `<div class="nz-feed__more">${tf('feed.moreNotShown', { n: filtered.length - MAX_FEED_ITEMS })}</div>`
    : '';
  ...
  return `
    <div class="nz-panel" id="nz-recent-feed">
      <div class="nz-panel__header">
        <span class="nz-panel__title">${t('feed.title')}</span>
        ${rangeLabel}
      ...
      <div class="nz-feed__list">
        ${items || `<div class="nz-feed__empty">${t('feed.noEvents')}</div>`}
        ${moreLabel}
      </div>
    </div>
  `;
...
export function mountRecentFeed(
  ...
  function render(): void {
    ...
    container.querySelectorAll<HTMLInputElement>('.nz-feed__range-input').forEach((input) => {
      input.addEventListener('change', () => {
        ...
        if (hint) {
          if (fromMs >= toMs) {
            hint.textContent = t('feed.invalidRange');
            hint.classList.add('nz-feed__range-hint--error');
          } else if (spanDays > MAX_CUSTOM_RANGE_DAYS) {
            hint.textContent = tf('feed.rangeTooLong', { n: Math.round(spanDays) });
            hint.classList.add('nz-feed__range-hint--error');
          } else {
            hint.textContent = tf('feed.daysSelected', { n: Math.round(spanDays) });
  ...
  const unsub4 = onLocaleChange(() => scheduleRender());
