...
export function createNotificationQueue(
      ...
      <div class="nz-notif nz-notif--${n.severity}${n.event ? ' nz-notif--clickable' : ''}" data-notif-id="${n.id}">
        ...
        <div class="nz-notif__content">
          <div class="nz-notif__header">
            <span class="nz-notif__severity">${localizedSeverityLabel(n.severity)}</span>
            <button class="nz-notif__dismiss" data-dismiss="${n.id}">×</button>
          </div>
          <div class="nz-notif__title">${escapeHtml(n.title)}</div>
          <div class="nz-notif__detail">${escapeHtml(n.detail)}</div>
        </div>
      </div>
    `).join('');
  ...
  const unsubEvents = consoleStore.subscribe('events', (events) => {
    ...
    for (const event of newEvents) {
      ...
      push({
        ...
        severity,
        title: tf('notif.detected', { mag: event.magnitude.toFixed(1) }),
        detail: escapeHtml(getLocalizedPlace(event.lat, event.lng, event.place.text)),
        timestamp: Date.now(),
        event,
      });
  ...
  // Realtime degradation toast suppressed — operators see freshness dots in system bar.
  const unsubRealtime = () => {};
