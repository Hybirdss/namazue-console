...
const MAX_HISTORY_PER_EVENT = 12;

function isCanonicalEventSource(value: EarthquakeEvent['source']): value is CanonicalEventSource {
  return value === 'server'
    || value === 'usgs'
    || value === 'jma'
    || value === 'historical'
  ...
  upsert(events: EarthquakeEvent[], options: EarthquakeUpsertOptions = {}): { added: number; updated: number } {
    ...
    for (const event of events) {
      ...
      const incoming = buildCanonicalEventEnvelope({
        event,
        source: isCanonicalEventSource(event.source) ? event.source : (options.source ?? 'server'),
      ...
      this.historyById.set(
        event.id,
        mergeCanonicalEventHistory(history, incoming, MAX_HISTORY_PER_EVENT),
      );
      if (existing) {
        const decision = resolvePreferredEventEnvelope(existing, incoming);
