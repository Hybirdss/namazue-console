...
import { R2_FEED_BASE } from "./config";
...
export function resolveDynamicSectionData(input: {
	...
	result: Pick<
		UnifiedPollResult,
		"source" | "vessels" | "railStatuses" | "domainOverrides" | "realtimeStatus"
	>;
}): Pick<UnifiedPollResult, "vessels" | "railStatuses" | "domainOverrides"> {
	const preserveAll = input.result.source === "fallback";
	const maritimeState = input.result.realtimeStatus.components?.find(
		(component) => component.id === "maritime",
	)?.state;
	const railState = input.result.realtimeStatus.components?.find(
		(component) => component.id === "rail",
	)?.state;
	const preserveVessels =
		preserveAll ||
		(input.result.vessels.length === 0 &&
			(maritimeState === "degraded" ||
				maritimeState === "down" ||
				maritimeState === "unknown"));
	const preserveRailStatuses =
		preserveAll ||
		(input.result.railStatuses.length === 0 &&
			(railState === "degraded" ||
				railState === "down" ||
				railState === "unknown"));
	const preserveDomainOverrides =
		preserveAll ||
	...
	return {
		...
		railStatuses: preserveRailStatuses
			? input.previousRailStatuses
			: input.result.railStatuses,
		domainOverrides: preserveDomainOverrides
			? input.previousDomainOverrides
			: input.result.domainOverrides,
	};
}
...
function extractDomainOverrides(
	snapshot: SnapshotResponse,
): OperatorBundleDomainOverrides {
	if (
		!snapshot.domain_overrides ||
		typeof snapshot.domain_overrides !== "object"
	) {
		return {};
	}
...
function deriveProjectionFreshness(
	snapshot: SnapshotResponse,
): UnifiedPollResult["projectionFreshness"] {
	return {
		events:
			snapshot.sections?.events?.updated_at ??
			snapshot.source_updated?.events ??
			snapshot.generated_at ??
			0,
		maritime:
			snapshot.sections?.maritime?.updated_at ??
			snapshot.source_updated?.maritime ??
			snapshot.generated_at ??
			0,
		rail:
			snapshot.sections?.rail?.updated_at ??
			snapshot.source_updated?.rail ??
			snapshot.generated_at ??
			0,
	};
}
...
function buildSnapshotSectionComponents(snapshot: SnapshotResponse) {
	const sections: Array<
		[
			...
			(
				| NonNullable<SnapshotResponse["sections"]>[ProjectionSectionName]
				| undefined
			),
		]
	> = [
	...
	return sections.map(([name, section]) => ({
		...
		message:
			section?.last_error ??
			(section?.state === "stale"
				? tf("poller.staleMessage", { section: formatSectionLabel(name) })
				: undefined),
	}));
}
...
function buildProjectionRealtimeStatus(
	snapshot: SnapshotResponse,
	freshness: UnifiedPollResult["projectionFreshness"],
): RealtimeStatus {
	...
	const degradedSection = sections.find(
		(entry) => entry[1]?.state === "degraded" || entry[1]?.state === "down",
	);
	if (degradedSection) {
		...
		return {
			...
			message: section?.last_error
				? tf("poller.degradedMessage", {
						section: formatSectionLabel(name as ProjectionSectionName),
						state: section?.state ?? "degraded",
						error: section.last_error,
					})
				: tf("poller.degradedNoError", {
						section: formatSectionLabel(name as ProjectionSectionName),
						state: section?.state ?? "degraded",
					}),
			components,
	...
	if (staleSection) {
		...
		return {
			...
			updatedAt: freshness.events || snapshot.generated_at || Date.now(),
			staleAfterMs: section?.stale_after_ms ?? 60_000,
			message: tf("poller.staleMessage", {
				section: formatSectionLabel(name as ProjectionSectionName),
			}),
			components,
		};
	...
	async function fetchSnapshot(): Promise<UnifiedPollResult> {
		...
		if (snapshotBaseUrl) {
			try {
				...
				if (snapshot && Array.isArray(snapshot.events)) {
					...
					const realtimeStatus = buildProjectionRealtimeStatus(
						snapshot,
						projectionFreshness,
					);
					return {
		...
		if (options.fallbackFetchEvents) {
			try {
				...
				} = await options.fallbackFetchEvents();
