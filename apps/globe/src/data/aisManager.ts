...
import { API_BASE as AIS_API_BASE, R2_FEED_BASE } from "./config";

export type { AisCoverageProfile, AisCoverageProfileId, Vessel, VesselType };
...
const AIS_R2_FEED = R2_FEED_BASE ? `${R2_FEED_BASE}/feed/maritime.json` : "";
const AIS_COVERAGE_PROFILE_ID = (
	import.meta.env.VITE_AIS_COVERAGE_PROFILE as string | undefined
)?.trim();
...
export function getAisCoverageProfile(
	profileId: AisCoverageProfileId,
): AisCoverageProfile {
...
export function resolveAisManagerConfig(
	...
	const profileId =
		options.profileId ?? parseAisCoverageProfileId(AIS_COVERAGE_PROFILE_ID);
	const profile = getAisCoverageProfile(profileId);
	return {
		apiBase: (options.apiBase ?? AIS_API_BASE ?? null)?.trim() || null,
		profile,
		demoFleetScale: clampDemoScale(
			options.demoFleetScale ?? AIS_DEMO_SCALE,
			profile.demoFleetScale,
		),
	};
}
...
function interpolateLane(
	...
	for (let i = 1; i < waypoints.length; i++) {
		...
		if (target <= segLen) {
			...
			return [waypoints[i - 1][0] + dx * f, waypoints[i - 1][1] + dy * f];
		}
		target -= segLen;
	}
...
function laneHeading(waypoints: [number, number][], t: number): number {
	const dt = 0.001;
...
function advanceFleet(
	...
	for (const vessel of fleet) {
		...
		const [baseLng, baseLat] = interpolateLane(
			lane.waypoints,
			vessel._progress,
		);
		...
		if (
			Math.abs(nextLng - prevLng) > 0.0001 ||
			Math.abs(nextLat - prevLat) > 0.0001
		) {
...
function createApiAisManager(
	...
	async function poll(): Promise<void> {
		...
		try {
			...
			// Maritime API unavailable — fall back to synthetic fleet
			if (!fallbackManager) {
				fallbackManager = createDemoAisManager(
					profile,
					profile.demoFleetScale,
					onUpdate,
				);
