...
export const LAB_TABS: LabTab[] = [
  ...
  },
  {
    ...
    title: 'Console runtime and lab workbench are deliberately separated.',
    body: 'The point of the split is maintenance quality. Product code stays in the console runtime while review material lives in the workbench.',
  },
  {
    id: 'voice',
    label: 'Voice',
...
export const ARCHITECTURE_CARDS: ArchitectureCard[] = [
  {
    ...
    body: 'The route split is explicit so the product runtime and review surface stop contaminating each other.',
    items: ['`/` -> operator console', '`/lab` -> workbench tabs'],
  },
  {
    title: 'Bootstrap Boundary',
    body: 'Entry resolution stays thin. The lab workbench is opt-in, and the console remains the default runtime.',
    items: ['`src/entry.ts` resolves pathname', '`src/core/bootstrap.ts` owns console startup', '`src/namazue/app.ts` owns workbench startup'],
  },
  {
  ...
  },
  {
    ...
    body: 'No new route should depend on dead product branches. Shared code must be tiny and explicit.',
    items: ['No route-specific DOM cross-talk', 'No workbench-only imports from console runtime', 'Static review content lives in typed registries'],
  },
  {
