...
export interface ImpactPlaybookPort {
  name: string;
  nameJa: string;
  lat: number;
  lng: number;
...
export interface ImpactResponseMilestoneTemplate {
  id: string;
...
const DEFAULT_MAJOR_PORTS: ImpactPlaybookPort[] = [
...
const DEFAULT_RESPONSE_MILESTONES: ImpactResponseMilestoneTemplate[] = [
...
export let MAJOR_PORTS: ImpactPlaybookPort[] = [...DEFAULT_MAJOR_PORTS];
export let RESPONSE_MILESTONE_TEMPLATES: ImpactResponseMilestoneTemplate[] = [
  ...DEFAULT_RESPONSE_MILESTONES,
];

interface RuntimePlaybookResponse {
  ports: ImpactPlaybookPort[];
  response_timeline: ImpactResponseMilestoneTemplate[];
}

export async function loadImpactPlaybook(): Promise<number> {
  const payload = await fetchReferenceJson<RuntimePlaybookResponse>('/api/runtime/playbook')
    ?? await fetchReferenceJson<RuntimePlaybookResponse>('/api/reference/playbook');

  if (payload?.ports && payload?.response_timeline) {
    MAJOR_PORTS = [...payload.ports];
    RESPONSE_MILESTONE_TEMPLATES = [...payload.response_timeline];
  }

  return RESPONSE_MILESTONE_TEMPLATES.length;
}
